...
group_factoring = parser.add_mutually_exclusive_group()
group_factoring.add_argument("-u", "--posfactored",
    help="factor output according to UPOS (default)",
    action="store_true")
group_factoring.add_argument("-d", "--deprelfactored",
    help="factor output according to deprel instead of UPOS",
    action="store_true")
group_factoring.add_argument("-j", "--joined",
    help="joined nonfactored eval",
    action="store_true")

group_factoring2 = parser.add_mutually_exclusive_group()
group_factoring2.add_argument("-p", "--predicted",
    help="factor according to predicted labels (default)",
    action="store_true")
group_factoring2.add_argument("-g", "--gold",
    help="factor according to gold labels instead of predicted",
    action="store_true")

group_measure = parser.add_mutually_exclusive_group()
group_measure.add_argument("-a", "--accuracy",
    help="individual accuracy (default)",
    action="store_true")
group_measure.add_argument("-e", "--errorshare",
...
UPOS = [
       ...
       'AUX','PUNCT', 'CONJ', 'CCONJ', 'PART', 'NUM', 'SCONJ', 'X', 'INTJ', 'SYM'
        ]
...
# TODO add a switch for ud1 vs ud2
UDEP = [
       'punct', 'nmod', 'obl', 'case', 'nsubj', 'det', 'root', 'dobj', 'obj', 'compound',
       'advmod', 'amod', 'conj', 'mark', 'cc', 'aux', 'cop', 'advcl', 'acl',
       'xcomp', 'nummod', 'ccomp', 'neg', 'appos', 'parataxis', 'auxpass',
       'name', 'nsubjpass', 'discourse', 'expl', 'mwe', 'fixed', 'flat', 'list', 'iobj',
       'csubj', 'goeswith', 'vocative', 'remnant', 'orphan', 'reparandum', 'dep',
       'csubjpass', 'foreign', 'dislocated', 'clf'
        ]
