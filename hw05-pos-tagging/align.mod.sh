...
paste data/en.s.mod data/kk.s.mod | sed 's/\t/ ||| /' | grep '. ||| .' > alignments.mod/en-kk
$fast_align -d -o -v -i alignments/en-kk > alignments/en-kk.f
$fast_align -d -o -v -r -i alignments.mod/en-kk > alignments.mod/en-kk.r
$atools -i alignments/en-kk.f -j alignments/en-kk.r -c intersect > alignments.mod/en-kk.i
echo "Aligning English-Kazakh done."

echo "Aligning Turkish-Kazakh pair"
paste data/tr.s data/kk.s | sed 's/\t/ ||| /' | grep '. ||| .' > alignments/tr-kk
...
paste data/ru.s data/kk.s | sed 's/\t/ ||| /' | grep '. ||| .' > alignments/ru-kk
